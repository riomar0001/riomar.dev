      ...
      <div className="relative mx-auto max-w-5xl">
        ...
        <div className="space-y-4">
          {skillGroups.map((group) => (
            <div key={group.id} className="group relative rounded-2xl border border-neutral-100 bg-white p-5 transition-all duration-300 hover:border-emerald-300 hover:shadow-xl hover:shadow-emerald-500/10 dark:border-neutral-800/50 dark:bg-neutral-900/80 dark:hover:border-emerald-500/50">
              ...
              <div className="flex flex-wrap gap-2">
                {group.items.map((item) => (
                  <span key={item.id} className="rounded-full bg-neutral-100 px-3 py-1 text-xs font-medium text-neutral-600 dark:bg-neutral-800 dark:text-neutral-400">{item.name}</span>
                ))}
              </div>
            </div>
          ))}
